...
# Dynamic transaction costs for stock trading (USD per share). Set to 0.0 to disable.
TC_PER_SHARE = 0.0035
...
def payoff(S1, S2):
    return 0.5 * (S1 + S2) - np.sqrt(S1 * S2)
...
def hedgeGapOnLargeGrid(df_t0, t0, t1, t2, S1_N, S2_N, parameters):
    ...
    for i in range(len(S2_N)):
        M_S1_S0[:, i] = (S1_N - S0 * IR_effect(t0, t0, t1)) * IR_effect(t0, t1, t2)
    ...
    M_hedge = (
            ...
            - IR_effect(t0, t0, t2) * TC_PER_SHARE * abs(parameters['delta0'])
            - IR_effect(t0, t1, t2) * TC_PER_SHARE * np.abs(parameters['delta1'](S1_N) - parameters['delta0']).reshape(len(S1_N), 1)
...
def params(df_t0, t0, t1, t2, S1, S2, init=None):
    ...
    # --- transaction cost linearization (per-share cost on stock trading) ---
    # Cost is paid when changing stock position: |H0| at t0 and |H1(S1)-H0| at t1.
    abs_delta0 = m.addVar(lb=0.0, name="abs_delta0")
    m.addGenConstrAbs(abs_delta0, delta0['delta0'], "abs_delta0_constr")
    diff_delta = m.addVars(np.array(S1).flatten(), lb=-float('inf'), name="diff_delta")
    abs_diff_delta = m.addVars(np.array(S1).flatten(), lb=0.0, name="abs_diff_delta")
    for _s1 in np.array(S1).flatten():
        m.addConstr(diff_delta[_s1] == delta1[_s1] - delta0['delta0'], name=f"diff_delta_constr[{_s1}]")
        m.addGenConstrAbs(abs_diff_delta[_s1], diff_delta[_s1], f"abs_diff_delta_constr[{_s1}]")
    ...
    # Objective is the initial (t0) setup cost. For the sub-hedging bound (lower bound),
    # we maximize the admissible initial cost subject to terminal dominance constraints.
    m.setObjective(
        d['d']
    ...
    for S1_i in S1:
        for S2_j in S2:
            ...
            # Sub-hedge: portfolio terminal value must not exceed payoff on the grid.
            m.addConstr(IR_effect(t0, t0, t2) * d['d']
                        ...
                        + IR_effect(t0, t1, t2) * delta0['delta0'] * (S1_i - S0 * IR_effect(t0, t0, t1))
                        + delta1[S1_i] * (S2_j - S1_i * IR_effect(t0, t1, t2))
                        - IR_effect(t0, t0, t2) * TC_PER_SHARE * abs_delta0
                        - IR_effect(t0, t1, t2) * TC_PER_SHARE * abs_diff_delta[S1_i]
...
def cuttingPlane(df_t0, t0, t1, t2, N, n, realS1, realS2, TOL):
    ...
    # We work with gap = hedge - payoff.
    # For sub-hedging we require hedge <= payoff on the grid, i.e. max(gap) <= 0.
    delta = np.infty
    k = 0
    p = None
    while delta >= TOL:
        p = params(df_t0, t0, t1, t2, S1, S2, init=p)
        if p == False:
...
def hedgingStrategy(parameters, df_t0, t0, t1, t2, s1, s2):
    ...
    h = (IR_effect(t0, t0, t2) * parameters['d']
         ...
         + sum(IR_effect(t0, t2, t2) * (parameters['theta2_ask'][r] - parameters['theta2_bid'][r]) * max(s2 - K2[r], 0)
               for r in df_t0.dropna().index)
         + IR_effect(t0, t1, t2) * parameters['delta0'] * (s1 - S0 * IR_effect(t0, t0, t1))
         + parameters['delta1'](s1) * (s2 - s1 * IR_effect(t0, t1, t2))
         - IR_effect(t0, t0, t2) * TC_PER_SHARE * abs(parameters['delta0'])
         - IR_effect(t0, t1, t2) * TC_PER_SHARE * abs(parameters['delta1'](s1) - parameters['delta0'])
...
for filename in sorted(data_files):
    ...
    for t0 in t0List:
        ...
        S0 = float(df_t0['Adj_S0'].iloc[0])
        parameters = cuttingPlane(df_t0, t0, t1, t2, N, n, realS1, realS2, tol*S0)
        if parameters == False:
            gap_S0 = np.array(np.nan)
        else:
            h = hedgingStrategy(parameters, df_t0, t0, t1, t2, realS1, realS2)
    ...
    df_gap_S0.to_csv(f'tmp_results/gapData_sub_{ticker}_{date.today().strftime("%d%m%y")}.csv')
